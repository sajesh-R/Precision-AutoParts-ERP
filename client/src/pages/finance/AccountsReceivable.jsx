...
const AccountsReceivable = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get('/finance/ar');
  ...
  const handleSaveInvoice = async (e) => {
    e.preventDefault();
    try {
      await axios.post('/finance/ar', formData);
  ...
  const handleSavePayment = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`/finance/ar/${formData._id}/payment`, {
  ...
  const actionRenderer = (row) => {
    if (activeTab === 'invoices') return null; // No edit after creation in this simple flow
    if (activeTab === 'payments' && row.status !== 'Paid') {
      return (
        <button onClick={() => { setFormData({...row, paymentAmount: row.outstandingAmount}); setIsPaymentModalOpen(true); }} className="btn-icon" style={{ fontSize: '11px', color: '#10b981', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
          <DollarSign size={12} /> Receive Payment
        </button>
      );
    }
    return null;
  ...
  const invoiceColumns = [
    ...
    { header: 'Date', render: (row) => new Date(row.invoiceDate).toLocaleDateString() },
  ...
  const getActiveData = () => {
    if (activeTab === 'outstanding') return invoices.filter(i => i.status !== 'Paid');
    return invoices;
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '16px', borderBottom: '1px solid var(--border-color)', marginBottom: '16px' }}>
        <div>
          <h1 style={{ fontSize: '18px', fontWeight: 600 }}>Accounts Receivable</h1>
          <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>Manage customer invoices, track incoming payments, and monitor outstanding dues</p>
        </div>
        {activeTab === 'invoices' && (
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '16px', borderBottom: '1px solid var(--border-color)' }}>
        <button onClick={() => setActiveTab('invoices')} style={{ padding: '8px 16px', background: 'none', border: 'none', borderBottom: activeTab === 'invoices' ? '2px solid var(--accent-primary)' : '2px solid transparent', color: activeTab === 'invoices' ? 'var(--accent-primary)' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: 500, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <FileText size={16} /> Customer Invoices
        </button>
        ...
        <button onClick={() => setActiveTab('outstanding')} style={{ padding: '8px 16px', background: 'none', border: 'none', borderBottom: activeTab === 'outstanding' ? '2px solid var(--accent-primary)' : '2px solid transparent', color: activeTab === 'outstanding' ? 'var(--accent-primary)' : 'var(--text-secondary)', cursor: 'pointer', fontWeight: 500, fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <TrendingUp size={16} /> Outstanding Monitoring
        </button>
      </div>
      ...
      {/* Invoice Modal */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Create Customer Invoice">
        <form onSubmit={handleSaveInvoice}>
          ...
          <div className="input-group">
            ...
            <input type="number" step="0.01" className="input-field" required value={formData.taxAmount || 0} onChange={e => setFormData({...formData, taxAmount: Number(e.target.value)})} />
          </div>
      ...
      {/* Payment Modal */}
      <Modal isOpen={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} title="Record Customer Payment">
        <form onSubmit={handleSavePayment}>
          <div style={{ backgroundColor: 'var(--bg-tertiary)', padding: '12px', borderRadius: '6px', marginBottom: '16px', fontSize: '13px' }}>
            <strong>Invoice #:</strong> {formData.invoiceNumber} <br />
