...
import { Plus, Boxes, Tags, CheckCircle, XCircle } from 'lucide-react';
...
const MaterialMaster = () => {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get('/master/materialcategory');
  ...
  const tabs = [
    { id: 'materials', label: 'Materials', icon: <Boxes size={18} /> },
    { id: 'categories', label: 'Material Categories', icon: <Tags size={18} /> },
  ];
              ...
              <div className="input-group">
                ...
                {categories.length === 0 && (
                  <p style={{ color: 'var(--accent-danger)', fontSize: '11px', marginTop: '4px' }}>
                    * Please create a Material Category first.
                  </p>
                )}
              </div>
              <div className="input-group">
